...
int client::location_request(std::string file_name, std::vector<chunk>& chunks, int & file_size){
    ...
    */
    ...
    struct chunk temp_chunk;    //{index,isPossesed,vector<peer> peers}
    struct peer temp_peer;  //{address,port}
   ...
   //store the routing information into client's data structure.
    for(int i=0; i<number_of_location; i++){    // for each peer
        // get peers
...
/*
#include <filesystem>
bool client::file_exist(const std::string& file_name) {
...
std::vector<int> client::select_chunk(std::vector<chunk> chunks){
    ...
    int min_index = 0;
    bool flag = false;
    for(int i = 0; i<4; i++){   // select atmost 4 chunks
...
int client::download(std::string file_name) {
    ...
    // Get file location
    if (location_request(file_name, chunks, file_length) < 0) {
    ...
    while (!indexes.empty()) {
        for (int i = 0; i < indexes.size(); i++) {
            if (indexes[i] < 0 || indexes[i] >= chunks.size()) {
                std::cerr << "Invalid chunk index: " << indexes[i] << std::endl;
                continue;
            }

            // Download chunk
            std::thread download_thread(
                &client::download_file, myclient, file_name, indexes[i], chunks[indexes[i]]);
            download_thread.detach();

            // Update logs and register chunk
            write_log(file_name, chunks);
            chunks[indexes[i]].is_possessed = true;
            register_chunk(file_name, indexes[i]);
        }
...
int client::execute(void){
    ...
    peer_address.sin_port = 0; // Let the OS choose an available port
    ...
    if (getsockname(server_fd, (struct sockaddr*)&peer_address, &len) < 0) {
        perror("getsockname failed");
        exit(1);
    }
    std::cout << "Client bound to port: " << ntohs(peer_address.sin_port) << std::endl;
