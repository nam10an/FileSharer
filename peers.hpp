...
*/
    private:
        ...
        int download_file(std::string remote_name, std::string local_name, int index, struct chunk chunk_instance);
        ...
        int download(std::string remote_name, std::string local_name);
    ...
    public:
        // void user_interface();
        void user_interface();  // keep this for internal use
        // static void user_interface(client* self);  // for thread
        int port;
        client(int p) : port(p) {}  
